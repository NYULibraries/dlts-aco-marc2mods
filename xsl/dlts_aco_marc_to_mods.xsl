<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
	...
	<!-- Maintenance note: For each revision, change the content of <recordInfo><recordOrigin> to reflect the new revision number.
                2014-11-23 jgpawletko : 
	        commented out <note> and </note> around <xsl:call-template name="relatedItem510"/> as it was creating invalid MODS.
	     
		Esha Datta: Adapted from MARC21slim2MODS3.xsl
		1. Tweaked script labels to reflect actual data
	-->
...
<!-- 2014-11-24 jgp				</note> -->
	...
	<xsl:template name="scriptCode">
		...
		<xsl:variable name="scriptCode" select="substring($sf06, 1, 3)"/>
		<xsl:if test="//marc:datafield/marc:subfield[@code='6']">
			<xsl:attribute name="script">
				<xsl:choose>
					<xsl:when test="contains($scriptCode,'880')">Latn</xsl:when>
					<xsl:when test="not(contains($scriptCode,880))">Arab</xsl:when>
	...
	<xsl:template name="createNoteFrom245c">
		<xsl:choose>
			...
			<xsl:when
				test="marc:subfield[@code='c'] and //marc:datafield[@tag='245'] and //marc:datafield[@tag=880]/marc:subfield[@code=6][contains(text(),'245')]">
